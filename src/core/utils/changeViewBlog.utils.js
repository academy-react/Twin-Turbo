...
//   rowView = result.news.miniDescribe;
//   colView = result.news.miniDescribe;
...
const changeViewBlog = async (parent) => {
  ...
  for (let i = 0; i < children.length; i++) {
    if (radios1.checked == true) {
      parent.current.children[i].lastChild.firstChild.lastChild.innerHTML = result.news[i].miniDescribe;
